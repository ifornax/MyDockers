...
BAND="VV" # band under investigation
FREQ="UHF"
...
CFGFILE="$HOME/KATHPRFI/RDB_FILES/kathprfi_config_file_template_Uband_Jan_Feb_2023_reportVV.txt"
# These are for output

OUTZARR="$HOME/KATHPRFI/OUT_ZARR/${FREQ}_${BAND}_.zarr"
GOODFILES="$HOME/KATHPRFI/GOOD_FILES/good_files_${FREQ}_${BAND}_Visitor.npy"
BADFILES="$HOME/KATHPRFI/BAD_FILES/bad_files_${FREQ}_${BAND}_Visitor.npy"
...
alias kathprfiUbandvv="docker run ${DOCKER_OPTS} --name $DOCKER_NAME $DOCKER_NAME python3 $SCRIPT -c $CFGFILE -z $OUTZARR -b $BADFILES -g $GOODFILES"

echo "Type kathprfiLbandvv to run the process..."
